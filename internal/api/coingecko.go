...
// FetchSupportedCurrencies returns the list of vs_currency values supported by CoinGecko.
func (c *HTTPClient) FetchSupportedCurrencies(ctx context.Context) ([]string, error) {
	...
	if resp.StatusCode == http.StatusTooManyRequests {
		...
		rle := &RateLimitError{Body: string(body)}
		if retryAfterHeader := resp.Header.Get("Retry-After"); retryAfterHeader != "" {
			if seconds, err := strconv.Atoi(retryAfterHeader); err == nil {
				rle.RetryAfter = time.Duration(seconds) * time.Second
			}
	...
	if err := json.NewDecoder(resp.Body).Decode(&codes); err != nil {
		return nil, fmt.Errorf("decoding supported currencies: %w", err)
	}
